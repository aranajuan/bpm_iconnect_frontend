...
class UploadHandler {
    ...
    protected $error_messages = array(
        ...
        3 => 'No se pudo completar la carga del archivo.',
        4 => 'No se subio el archivo.',
        6 => 'No se encuentra carpeta temporal.',
        7 => 'No se pudo generar el archivo en el servidor.',
        8 => 'A PHP extension stopped the file upload',
        'post_max_size' => 'The uploaded file exceeds the post_max_size directive in php.ini',
        'max_file_size' => 'El archivo es muy grande.',
        'min_file_size' => 'El archivo es muy chico.',
        'accept_file_types' => 'Tipo de archivo no permitido.',
        'max_number_of_files' => 'Se supero el maximo numero de archivos permitidos.',
        'max_width' => 'Image exceeds maximum width',
        'min_width' => 'Image requires a minimum width',
        'max_height' => 'Image exceeds maximum height',
        'min_height' => 'Image requires a minimum height',
        'abort' => 'Se cancelo la carga del archivo',
        'image_resize' => 'No se pudo generar imagen'
    );
    protected $image_objects = array();
    private $user;

    function __construct($options = null, $initialize = true, $error_messages = null) {
        $this->user = $GLOBALS["U"]->get_prop('hash');
        $this->options = array(
            'script_url' => $this->get_full_url() . '/',
            'upload_dir' => FILEUP_TMP_FOLDER . "/",
            'upload_url' => FILEUP_TMP_FOLDER . "/",
            'user_dirs' => false,
            'mkdir_mode' => 0755,
            'param_name' => 'files',
            ...
            'accept_file_types' => '/\.(' . FILEUP_ALLOWED_FORMATS . ')$/i',
            // The php.ini settings upload_max_filesize and post_max_size
            // take precedence over the following max_file_size setting:
            'max_file_size' => FILEUP_MAX_FILES_SIZE,
            'min_file_size' => 1,
    ...
    protected function validate($uploaded_file, $file, $error, $index) {
        ...
        if (!preg_match($this->options['accept_file_types'], $file->name)) {
            $file->error = $this->get_error_message('accept_file_types') . $file->name;
            return false;
        }
    ...
    private function getfile_nro() {
        $directorio = opendir(FILEUP_TMP_FOLDER);
        $foundIDs = array();
        $i = 0;
        ...
        closedir($directorio);
        if ($i == FILEUP_MAX_FILES)
            return -1;
        for ($i = 1; $i <= FILEUP_MAX_FILES; $i++) {
            if (!isset($foundIDs[$i])) {
                return $i;
            }
        }
        ...
        if ($upload && is_array($upload['tmp_name'])) {
            ...
            foreach ($upload['tmp_name'] as $index => $value) {
                ...
                $files[] = $this->handle_file_upload(
                        $upload['tmp_name'][$index], $fname, $size ? $size : $upload['size'][$index], $upload['type'][$index], $upload['error'][$index], $index, $content_range
                );
            }
        } else {
            ...
            $fname = ($file_name ? $file_name : (isset($upload['name']) ?
                                    $upload['name'] : null));
            $files[] = $this->handle_file_upload(
                    isset($upload['tmp_name']) ? $upload['tmp_name'] : null, $fname, $size ? $size : (isset($upload['size']) ?
